...
class GraphDataset(Dataset):
    ...
    def loadGraphs(path):
        ...
        graphs = []
        for graph_dict in tqdm(graphs_dicts, desc="Processing graphs", unit="graph", leave=False):
            graphs.append(dictToGraphObject(graph_dict))
        return graphs

class BatchedGraphDataset(Dataset):
    def __init__(self, filename, i, transform=None, pre_transform=None):
        self.raw = filename
        self.graphs = self.loadGraphs(self.raw)
        super().__init__(None, transform, pre_transform)
    ...
    def loadGraphs(path):
        with open(path, "r", encoding="utf-8") as f:
            lines = f.readlines()
            graphs_dicts = [json.loads(line) for line in lines]
        graphs = []
        for i, graph_dict in tqdm(enumerate(graphs_dicts), desc="Processing graphs", total=len(graphs_dicts), unit="graph", leave=False):
            graphs.append(dictToGraphObject(graph_dict, i))
        return graphs


def dictToGraphObject(graph_dict, i):
